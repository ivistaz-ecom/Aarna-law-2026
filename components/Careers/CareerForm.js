...
export default function CareerForm({ id }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      // Check file size (max 20MB as per Zoho requirements)
      const MAX_FILE_SIZE = 20 * 1024 * 1024; // 20MB
      if (file.size > MAX_FILE_SIZE) {
        setSubmitStatus({
          type: "error",
          message: "File size must be less than 20MB",
          allErrors: ["File size must be less than 20MB"],
        });
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch("/api/zoho/current-openings", {
        method: "POST",
            ...
            <div className="pointer-events-none">
              {fileName ? (
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <p className="text-xs text-gray-500">PDF, DOCX (Max 20MB)</p>
                </div>
              )}
            </div>
          </div>
