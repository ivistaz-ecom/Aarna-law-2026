...
function validateFileData(fileData, fileName) {
  ...
  // WHY: Enforce 5MB limit for internships
  const MAX_FILE_SIZE = 5 * 1024 * 1024; // 5MB
  const estimatedSize = (base64Data.length * 3) / 4;
  if (estimatedSize > MAX_FILE_SIZE) {
    errors.push(`File size exceeds 5MB limit. Please upload a smaller file.`);
    return { valid: false, errors };
  }
...
/**
 * Creates an Internships record in Zoho CRM
 * WHY: Separated for clarity and testability
 */
async function createInternshipRecord(payload, accessToken) {
  const zohoApiUrl = "https://www.zohoapis.in/crm/v2/Internships";
...
async function attachFileToRecord(recordId, fileData, fileName, accessToken) {
  try {
    ...
    const attachUrl = `https://www.zohoapis.in/crm/v2/Internships/${recordId}/Attachments`;
    ...
    let fileHash = null;
    if (fileId) {
      const attachmentsUrl = `https://www.zohoapis.in/crm/v2/Internships/${recordId}/Attachments`;
    ...
    if (fileIdentifier) {
      ...
      const updateUrl = `https://www.zohoapis.in/crm/v2/Internships/${recordId}`;
      const updatePayload = {
        data: [
          {
...
export async function GET() {
  ...
  return NextResponse.json({
    message: "Zoho Internships API route is accessible",
    endpoint: "/api/zoho/internships",
    method: "POST",
    timestamp: new Date().toISOString(),
    oauthConfigured: hasRefreshToken && hasClientId && hasClientSecret,
    zohoApiUrl: "https://www.zohoapis.in/crm/v2/Internships",
  });
}
...
export async function POST(request) {
  try {
    ...
    try {
      const result = await createInternshipRecord(payload, accessToken);
    ...
    console.error("❌ Error in Zoho Internships API route:", error);
