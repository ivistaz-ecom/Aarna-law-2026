...
function mapZohoError(errorCode, errorMessage, errorDetails) {
  const defaultMessage =
    errorMessage || "Failed to submit publication request. Please try again.";
...
export async function GET() {
  return NextResponse.json({
    message: "Zoho Publications API route is accessible",
    endpoint: "/api/zoho/publications",
    method: "POST",
    timestamp: new Date().toISOString(),
    zohoApiUrl: "https://www.zohoapis.in/crm/v8/Publications",
  });
}

/**
 * POST handler for publication form submission
 * WHY: Creates a record in Zoho CRM Publications module
 */
export async function POST(request) {
  try {
    ...
    // Zoho CRM API endpoint for Publications module (v8)
    const zohoApiUrl = "https://www.zohoapis.in/crm/v8/Publications";
      ...
      return NextResponse.json(
        {
          ...
          recordId: recordId,
          message: "Successfully submitted publication request",
        },
        { status: 200 },
      );
    }
    ...
    console.error("❌ Error in Zoho Publications API route:", error);
